...
def validate_pt():
    model = YOLO(str(PT))
    metrics = model.val(data=str(DATA_YAML), imgsz=IMG_SIZE, split="val", device=0 if torch.cuda.is_available() else "cpu", plots=True)
    return {
...
def export_onnx():
    ...
    model = YOLO(str(PT))
    exported = Path(model.export(format="onnx", opset=12, imgsz=IMG_SIZE, simplify=False, dynamic=False))
    final_pt = EXPORT / "best.pt"
    final_onnx = EXPORT / "best.onnx"
    shutil.copy2(PT, final_pt)
    shutil.copy2(exported, final_onnx)
    return final_pt, final_onnx


def benchmark_onnx(onnx_path, sample_count=59, warmup=10):
    providers = ["CPUExecutionProvider"]
    sess = ort.InferenceSession(str(onnx_path), providers=providers)
    input_meta = sess.get_inputs()[0]
    output_meta = sess.get_outputs()[0]
    imgs = list_images("test")
    random.seed(42)
    sample = random.sample(imgs, min(sample_count, len(imgs)))
    prepared = [preprocess(p) + (p,) for p in sample]

    for inp, *_ in prepared[: min(warmup, len(prepared))]:
        sess.run(None, {input_meta.name: inp})
    ...
    for path in sample:
        ...
        inp, img0, ratio, pad = preprocess(path)
        t1 = time.perf_counter()
        out = sess.run(None, {input_meta.name: inp})[0]
        t2 = time.perf_counter()
        det = decode(out, img0.shape, ratio, pad)
        ...
        if len(samples) < 5:
            samples.append({"image": str(path), "detections": det[:8]})
    ...
    return {
        ...
        "onnxruntime_providers_available": ort.get_available_providers(),
        ...
        "inference_ms": stats(infer_ms),
        "postprocess_ms": stats(post_ms),
        "total_ms": stats([a + b + c for a, b, c in zip(pre_ms, infer_ms, post_ms)]),
        "detections_per_image": stats(counts),
...
'''
    write_text(DEPLOY / "test_onnx_320.py", body)
...
def main():
    ...
    val = None if args.skip_val else validate_pt()
    final_pt, final_onnx = export_onnx()
    bench = benchmark_onnx(final_onnx)
    report = {
        "model": "YOLOv8n-320",
        "source_pt": str(PT),
        "exported_pt": str(final_pt),
        "exported_onnx": str(final_onnx),
        "onnx_size_mb": round(final_onnx.stat().st_size / 1024 / 1024, 3),
        "validation": val,
        "onnx_benchmark_local_cpu": bench,
        "note": "Local CPU latency is not RZ/G2L latency. Cortex-A55 should be measured on-device; this report verifies the model and ONNX path locally.",
    }
    write_text(VERIFY / "yolov8n_320_eval_export_report.json", json.dumps(report, indent=2, ensure_ascii=False))
    write_text(
        VERIFY / "yolov8n_320_eval_export_report.md",
        "# YOLOv8n-320 Evaluation and ONNX Export\n\n"
        f"PT: `{final_pt}`\n\n"
        f"ONNX: `{final_onnx}`\n\n"
        f"ONNX size: **{report['onnx_size_mb']} MB**\n\n"
        f"Validation: `{json.dumps(val, ensure_ascii=False)}`\n\n"
        f"Local CPU benchmark: `{json.dumps(bench['total_ms'], ensure_ascii=False)}`\n\n"
        f"Input: `{bench['input']}`\n\n"
        f"Output: `{bench['output']}`\n",
    )
    write_test_script()
